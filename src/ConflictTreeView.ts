...
export class ConflictTreeViewProvider implements vscode.TreeDataProvider<ConflictTreeItem> {
  constructor(
    private conflict: ConflictTreeItem[],
    private conflictIconPath: string,
    private resolvedIconPath: string,
  ) {
...
export async function conflictSectionsToTreeItem(
  ...
  for (const conflictSection of allConflictSections) {
    ...
    if (!flag) {
      const newParent = new ConflictTreeItem(
        undefined,
        conflict.uri,
        undefined,
        [newConflict],
